...
function Widgets({ articles }) {
  return (
    <div className="hidden xl:inline space-y-2">
      ...
      rounded-lg space-y-2 w-11/12 overflow-hidden border 
      border-gray-300 dark:border-none"
      >
        {/* Container for title linkedIn news */}
        <div className="flex items-center justify-between font-bold px-2.5">
          <h4>LinkedIn News</h4>
          <InfoRoundedIcon className="h-5 w-5" />
        </div>

        {/* Container for news from the props articles, and only using 5 of them */}
      ...
      {/* Ads, dummy Ad */}
      <div
        className="bg-white dark:bg-[#1D2226] w-11/12 h-64 px-2.5 
      rounded-lg sticky top-20 border border-gray-300 dark:border-none"
      >
